...
private: // function to calculate and print latency
      ...
      bool isHardcon, double *nStateSign)
  {
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      //printf(" ta[%d]=%f ",i,adjustedTargetAngle[i]);

      if (currentTime <= targetTime / 4.0)
      {
        // printf("%.5f+",v[i]);
        vSub[i] += a[i] * dt; // acceleration 25% of time
        v[i] += a[i] * dt; // acceleration 25% of time
        // printf("%.3f*%.4f=%.5f ",a[i],dt,v[i]);
        v[i] = vSub[i];
        set = 1;
      ...
      // integrate distance
      if (fabs(adjustedTargetAngle[i] - d[i]) < 0.006)
        d[i] = adjustedTargetAngle[i];
      else
        d[i] += v[i] * dt;

      //printf("ta[%d]=%.3f,d[%d]=%.3f,v[%d]=%.2f,a[%d]=%.2f  ",i,adjustedTargetAngle[i],i, d[i], i, v[i], i, a[i]);
    }
      ...
      bool isHardcon)
  {
    ...
    { // for gazebo simulation
      pos = 0;
      for (i = 0; i < 4; i++)
        uArm[i] = -.18 * (pArm[i] - angle[i] * targetAngle[i]) - .25 * (vArm[i] - 0); // 3.0 & 4.3
    ...
    // update currentTime
    currentTime = t - tstartmove;
    printf("ct=%.2f ta=%f  ", currentTime, targetAngle[0]);

    // end condition
    if ( // target position with allowance of 0.1
        (
            fabs(pWheel[0] - adjustedTargetPosition[0] ) < 0.1 ||
            fabs(pWheel[1] - adjustedTargetPosition[1] ) < 0.1 ||
            fabs(pWheel[2] - adjustedTargetPosition[2] ) < 0.1 ||
            fabs(pWheel[3] - adjustedTargetPosition[3] ) < 0.1) &&
        currentTime >= 0.5)
    {
      for (i = 0; i < 4; i++)
      {
        u[i] = 0; // set velocity back to 0 to stop motors running
      }
      isComplete = true;
      setInit = 1;
    }
      ...
      double *kp, double *ki)
  {
    ...
    case ROTATE:
    {
      ...
      if (moveRobot(s, v, pArm, uArm, t, d, pWheel, targetAngles, vArm, targetPosition, targetVelocity, isHardcon))
      {
...
private:
  void topic_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    case STEP8:
    {
      ...
      // calculate
      hypotenuse = sqrt( (pointX*pointX) + (pointY*pointY) );
      targetAngle = atan( pointX / pointY );

      // check if in y or x axis
      ...
      //1st & 3rd quadrant
      else if ( fabs(targetAngle) > M_PI / 4.0  ) {
        //targetAngle = M_PI / 2.0 - targetAngle;

        if ( pointX * pointY > 0 ) {
          targetAngles[0] = - targetAngle + M_PI;
          targetAngles[1] = targetAngle;
          targetAngles[2] = - targetAngle + M_PI;
          targetAngles[3] = targetAngle;
        ...
        //2nd & 4th quadrant
        else if (pointX * pointY < 0) {
          targetAngles[0] =  - targetAngle;
          targetAngles[1] =  targetAngle + M_PI;
          targetAngles[2] =  - targetAngle;
          targetAngles[3] =  targetAngle + M_PI;
      ...
      for(i=0; i<4; i++)
        printf(" ta[%d]=%.3f, ",i,adjustedTargetAngle[i]);

      

      // call swiveling function
      if (swivelEach(pArm, uArm, vArm, t, d, v, a, targetTime, targetAngles, isHardcon, s))
      {
    ...
    else if (pos == 3)
    {
      ...
      for (i = 0; i < 4; i++)
      {
        if(swivelControl[i]) {
          ...
          u[i] = -pGain * (pArm[i] - d[i]) - dGain * (vArm[i] - v[i]);
          u[i] += 0.015 * a[i];
          u[i] -= ESO[i];
    ...
    print_commands(uArm, uWheel, t, pState-1);
    //print_latency(t);
