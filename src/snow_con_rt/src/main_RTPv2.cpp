...
private: // function to calculate and print latency
      ...
      bool isHardcon, double *nStateSign)
  {
    ...
    static double wheelCompensation[4] = {0.02,0.01,-0.02,0.005};
    ...
    if (setInit)
    {
      ...
      for (i = 0; i < 4; i++)
      {
        ...
        if (isHardcon)
        {
          // if (targetAngle[i] >= M_PI / 2)
          // {
          //   if (nStateSign[i] > 0 && adjustedTargetAngle[i] > pArm[i])
          //     adjustedTargetAngle[i] += s[i] * wheelCompensation[i];
          ...
          // ONLY FOR ROTATION TO A POINT
          adjustedTargetAngle[0] += s[0] * wheelCompensation[0];
          adjustedTargetAngle[1] -= s[1] * wheelCompensation[1];
          adjustedTargetAngle[2] += s[2] * wheelCompensation[2];
          adjustedTargetAngle[3] -= s[3] * wheelCompensation[3];
    ...
    printf("ct=%.2f ta=%f", currentTime, fabs(targetAngle[0]));
    ...
    for (i = 0; i < 4; i++)
    {
      ...
      // integrate distance
      if (fabs(adjustedTargetAngle[i] - d[i]) < 0.01)
    ...
    for (i = 0; i < 4; i++)
    {
      if (fabs(adjustedTargetAngle[i] - pArm[i]) < 0.01 && !isEnd[i])
      {
        isEnd[i] = 1;
        uArm[i] = 0;
      }
      ...
      bool isHardcon)
  {
    ...
    { // for gazebo simulation
      pos = 0;
      for (i = 0; i < 4; i++)
        uArm[i] = -5 * (pArm[i] - angle[i] * targetAngle[i]) - 4.4721 * (vArm[i] - 0);
    }
    ...
    if ( // target position with allowance of 0.1
        (
            fabs(pWheel[0] - (s[0] * targetPosition[0] + d0[0])) < 0.1 ||
            fabs(pWheel[1] - (s[1] * targetPosition[1] + d0[1])) < 0.1 ||
            fabs(pWheel[2] - (s[2] * targetPosition[2] + d0[2])) < 0.1 ||
            fabs(pWheel[3] - (s[3] * targetPosition[3] + d0[3])) < 0.1) &&
        currentTime >= 0.5)
    {
      for (i = 0; i < 4; i++)
      {
    ...
    for (i = 0; i < 4; i++)
    {
      printf("%d=%.3f - %.3f\t", i, pWheel[i], s[i] * targetPosition[i] + d0[i]);
...
private:
  void topic_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    static double targetAngle[4];
    static double targetPosition[4];
    ...
    case STEP1:
    {
      ...
      // set target anlge
      targetAngle[0] = 0.10187363376;  // M_PI/4.0;
      targetAngle[1] = 0.10187363376;  // M_PI/4.0;
      targetAngle[2] = -0.12779965091; // M_PI/4.0;
      targetAngle[3] = -0.12779965091; // M_PI/4.0;
    ...
    case STEP2:
    {
      ...
      targetPosition[0] = ((2) * M_PI * (1.30518104764 / wheelRadius) / 1);
      targetPosition[1] = ((2) * M_PI * (1.30518104764 / wheelRadius) / 1);
      targetPosition[2] = ((2) * M_PI * (0.96659232171 / wheelRadius) / 1);
      targetPosition[3] = ((2) * M_PI * (0.96659232171 / wheelRadius) / 1);
      ...
      targetTime = targetPosition[0] / 3.0;

      targetVelocity[0] = 3.0;                            // set target torque
      targetVelocity[1] = 3.0;                            // set target torque
      targetVelocity[2] = targetPosition[2] / targetTime; // set target torque
      targetVelocity[3] = targetPosition[3] / targetTime; // set target torque
      ...
      // {
      //   // set target angle and position
      //   targetPosition = (0.4 / wheelRadius);
    ...
    if (pos == 1)
    {
      u[0] = -0.10 * (pArm[0] - d[0]) - 0.12 * (vWheel[0] * 0.33 - v[0]);
      u[0] -= eso0.fHat / eso0.b;
      u[0] += 0.025 * a[0];

      u[1] = -0.10 * (pArm[1] - d[1]) - 0.12 * (vWheel[1] * 0.33 - v[1]);
      u[1] -= eso1.fHat / eso1.b;
      u[1] += 0.025 * a[1];

      u[2] = -0.10 * (pArm[2] - d[2]) - 0.12 * (vWheel[2] * 0.33 - v[2]);
      u[2] -= eso2.fHat / eso2.b;
      u[2] += 0.025 * a[2];

      u[3] = -0.10 * (pArm[3] - d[3]) - 0.12 * (vWheel[3] * 0.33 - v[3]);
    ...
    else if (pos == 0)
    {
      ...
      {
        uWheel[0] = -0.02 * (e[0]) - 0.1 * inte[0] - 0.0 * de[0]; //-0.05*sin(2*M_PI*0.25*t)+0.0;// > 0 ? 1.0 : -1.0;//-0.4*(J0.dq - 2);
        uWheel[1] = -0.02 * (e[1]) - 0.1 * inte[1] - 0.0 * de[1]; //-0.05*sin(2*M_PI*0.25*t)+0.0;// > 0 ? 1.0 : -1.0;//-0.4*(J0.dq - 2);
        uWheel[2] = -0.02 * (e[2]) - 0.1 * inte[2] - 0.0 * de[2]; //-0.05*sin(2*M_PI*0.25*t)+0.0;// > 0 ? 1.0 : -1.0;//-0.4*(J0.dq - 2);
        uWheel[3] = -0.02 * (e[3]) - 0.1 * inte[3] - 0.0 * de[3]; //-0.05*sin(2*M_PI*0.25*t)+0.0;// > 0 ? 1.0 : -1.0;//-0.4*(J0.dq - 2);
      }
    }
