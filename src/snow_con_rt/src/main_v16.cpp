...
private: // function to calculate and print latency
      ...
      bool isHardcon)
  {
    ...
    for(i=0; i<4; i++) {
      if (fabs(s[i] * targetAngle - pArm[i]) < 0.03 && !isEnd[i]) {
        isEnd[i] = 1;
        uArm[i] = 0;
      }
    }
      ...
      bool isHardcon)
  {
    ...
    { // for gazebo simulation
      pos = 0;
      for (i = 0; i < 4; i++)
        uArm[i] = s[i]*(-3 * (pArm[i] - angle[i]*targetAngle) - 2.5 * (vArm[i] - 0));
    }
...
private:
  void topic_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    case STEP4:
    {
      ...
      // set target angle and position
      targetPosition = (0.2 / wheelRadius);
    ...
    if (pos == 1)
    {
      u[0] = -0.10 * (pArm[0] - d[0]) - 0.15 * (vWheel[0] * 0.33 - v[0]);
      u[0] -= eso0.fHat / eso0.b;
      u[0] += 0.035 * a[0];

      u[1] = -0.10 * (pArm[1] - d[1]) - 0.15 * (vWheel[1] * 0.33 - v[1]);
      u[1] -= eso1.fHat / eso1.b;
      u[1] += 0.035 * a[1];

      u[2] = -0.10 * (pArm[2] - d[2]) - 0.15 * (vWheel[2] * 0.33 - v[2]);
      u[2] -= eso2.fHat / eso2.b;
      u[2] += 0.035 * a[2];

      u[3] = -0.10 * (pArm[3] - d[3]) - 0.15 * (vWheel[3] * 0.33 - v[3]);
