...
private:
  void topic_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    case STEP8:
    {
      ...
      targetTime = 2.0;
      targetAngles[0] = 0;
      targetAngles[1] = -M_PI / 4.0;
      targetAngles[2] = 0;
      targetAngles[3] = -M_PI / 4.0;
      nStateSign[0] = 1.0;
      nStateSign[1] = 1.0;
      nStateSign[2] = 1.0;
      nStateSign[3] = 1.0;
      ...
      // call swiveling function
      if (swivelEach(pArm, uArm, t, d, v, a, targetTime, targetAngles, isHardcon, nStateSign))
      {
    ...
    case STEP9:
    {
      ...
      // set targets
      targetTime = 2.0;
      targetAngles[0] = M_PI / 4.0;
      targetAngles[1] = -M_PI / 4.0;
      targetAngles[2] = M_PI / 4.0;
      targetAngles[3] = -M_PI / 4.0;
      nStateSign[0] = 1.0;
      nStateSign[1] = 1.0;
      nStateSign[2] = 1.0;
    ...
    // swivel according to a point
    case STEP10:
    {
    
      targetX = 0.0;
      targetY = 1.0;
      quadrant = targetX * targetY; 
      
      
      if (targetY == 0.0) {
          targetAngles[0] = M_PI/2.0;  
          targetAngles[1] = M_PI/2.0;    
          targetAngles[2] = M_PI/2.0;    
          targetAngles[3] = M_PI/2.0;   
        ...
        if ( atan(targetX/targetY) > M_PI/4.0 && quadrant > 0) {
          
          targetAngles[0] = atan(targetX/targetY) + M_PI;
          targetAngles[1] = atan(targetX/targetY);  
          targetAngles[2] = atan(targetX/targetY) + M_PI;
          targetAngles[3] = atan(targetX/targetY);  
        }
        
        else if ( atan(targetX/targetY) > M_PI/4.0 && quadrant < 0) {
          targetAngles[0] = atan(targetX/targetY); 
          targetAngles[1] = atan(targetX/targetY) + M_PI;
          targetAngles[2] = atan(targetX/targetY);
          targetAngles[3] = atan(targetX/targetY) + M_PI;
        }
    ...
    case STEP11:
    {
      ...
      // set target angle and position
      targetPosition = ((sqrt(targetX*targetX + targetY*targetY)) / wheelRadius);
      targetAngle = 0; // simulation only
    ...
    else if (pos == 0)
    {
      for (i = 0; i < 4; i++)
      {
        ...
        uWheel[i] = -0.032 * (e[i]) - 0.1 * inte[i] - 0.0 * de[i];
      }
    }
