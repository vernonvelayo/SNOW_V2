      ...
      switch (pState) {
        ...
        case STEP2: {
          ...
          for (i = 0 ; i < 4 ; i++) {
            if (velocity != 0) //
              ...
              uArm[i] = -3*(pArm[i] - M_PI/2)- 2.5*(vArm[i] - 0);
          }

          // if it reaches 90deg
          if (fabs(pArm[0] - M_PI/2) < 0.01 && fabs(vArm[0]) < 0.01) {
        ...
        // robot will move 1m along +X axis
          ...
          for(i = 0 ; i < 4 ; i++) {  
            ...
            // wheel torque and arm torque (brake)
            uWheel[i] = -cPwheelForward*(pWheel[i]- d[i]) + -cVwheelForward*(vWheel[i]- v[i]);
            uArm[i] = -3*(pArm[i] - M_PI/2)- 0.3*(vArm[i] - 0);
          }
        ...
        case STEP4: {      
          ...
          if (fabs(pArm[0] - M_PI/4) < 0.02 && fabs(vArm[0]) < 0.02) {
            ...
            velocity = 0;
            position = 0;
            endposition = (2/2)*M_PI*(0.3064); // calculated mid of robot to tip of wheel
            wait = true;
        ...
        case STEP5: {
          ...
          for(i = 0 ; i < 4 ; i++) {  
            //if (t < tstartmove + 4) 
            if (s[i] > 0 && d[i] <=  s[i] * (endposition / rw) + dstartpos[i])
              //v[i] = s[i]*endposition/10/rw;
              v[i] = s[i]*0.6;
              //v[i] = (1 / (2*M_PI)) / 4;

            else if (s[i] < 0 && d[i] >=  s[i] * (endposition / rw) + dstartpos[i])
              //v[i] = s[i]*endposition/10/rw;
              v[i] = s[i]*0.6;
            ...
            // wheel torque and arm torque (brake)
            uWheel[i] = -0.1*(pWheel[i]- d[i]) + -0.1*(vWheel[i]- v[i]);
            uArm[i] = 0;//-0.2*(pArm[i] - M_PI/4) - 0.2*(vArm[i] - 0); 
          ...
          //printf("d[i]: %f", (endposition / rw) + dstartpos[1]);
