    ...
    void topic_callback(const sensor_msgs::msg::JointState::SharedPtr msg) 
    {
      ...
      switch (pState) {
        ...
        case STEP2: {
          ...
          uWheel[0] = (-cPwheelRotate*(pArm[0] - position) - cVwheelRotate*(vArm[0] - velocity));
          uWheel[1] = (-cPwheelRotate*(pArm[1] - position) - cVwheelRotate*(vArm[1] - velocity));
          uWheel[2] = -(-cPwheelRotate*(pArm[2] - position) - cVwheelRotate*(vArm[2] - velocity));
          uWheel[3] = -(-cPwheelRotate*(pArm[3] - position) - cVwheelRotate*(vArm[3] - velocity));

          for (i = 0 ; i < 4 ; i++) {
            if (t < tstartmove + 2) //
              uArm[i] = 0; 
            else
              uArm[i] = -3*(pArm[i] - M_PI/2)- 0.1*(vArm[i] - 0);
          }
